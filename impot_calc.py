...
with col_gauche:
    ...
    # Logique pour afficher le bon uploader en fonction du menu
...
with col_droite:
    ...
    # Lancement de l'analyse si des fichiers sont fournis
    ...
    if st.session_state.resultats_paie:
        with st.container(border=True):
            ...
            if res.get("fichiers_ignores"):
                with st.expander("Avertissements sur certains fichiers"):
                    for nom_fichier in res["fichiers_ignores"]:
                        st.warning(f"Le fichier '{nom_fichier}' n'a pas pu être traité (date/format invalide).")
    ...
    if st.session_state.resultats_ep5:
        with st.container(border=True):
            ...
            if res.get("warnings"):
                with st.expander("Notifications de l'analyse"):
                    for msg in res["warnings"]: st.info(msg)
            ...
            if res.get("has_results"):
                ...
                with tab_rot:
                    st.dataframe(res.get("rotations_df", pd.DataFrame()), hide_index=True, use_container_width=True)
                with tab_stats:
    ...
    if st.session_state.resultats_attestation:
        with st.container(border=True):
            ...
            if res.get("erreurs") or res.get("fichiers_sans_attestation"):
                with st.expander("Notifications de l'analyse"):
                    for err in res.get("erreurs", []): st.warning(err)
                    for f in res.get("fichiers_sans_attestation", []): st.info(f"Fichier '{f}': pas d'attestation trouvée.")
